...
export function CharacterPetSettings({ modelId, setFeedback }: Props) {
  ...
  const refreshStatus = useCallback(async (): Promise<{ hasAnimations: boolean; hasLines: boolean }> => {
  ...
  const actionTabs: { id: ActionSection; label: string; badge?: string }[] = [
    ...
    { id: "lines", label: "台词", badge: actionLayout.lines.length ? String(actionLayout.lines.length) : undefined },
