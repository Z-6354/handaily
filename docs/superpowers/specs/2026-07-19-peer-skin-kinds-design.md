...
| **P2** | 真分流 | 舰娘 Tab → 同窗 Cubism；桌宠 Tab → Spine | 只改点选分支；废除详情/设置里的「上桌/预览」旁路 |
| **P3** | 偏好正交 | 每角色或每槽记住「上次 kind」；冷启动恢复 | `active_kind`（或等价）与 `active_skin_id` 分列；`auto` 仅作迁移兼容 |
| **P4** | 资源真相 | 导入/同步只写 character 槽；kanmusu catalog 派生 | 收敛同步 API；设置页「同步」变成「扫描磁盘补全绑定」或并入导入 |
...
- P2：舰娘 Tab 点选 → 同窗 Cubism；桌宠 Tab → Spine；无旁路上桌按钮。  
- P3：重启后恢复「角色 + 皮肤槽 + kind」。  
- P4：一次导入/扫描即可补全绑定，用户无感双 manifest。  
