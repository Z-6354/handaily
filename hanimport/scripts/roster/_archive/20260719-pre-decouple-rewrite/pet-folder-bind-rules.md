...
| 位置 | 职责 |
|------|------|
| `hanimport/scripts/roster/db.py` | 模式规则、绑皮、`enrich_alias_map_from_roster` |
...
新增非常规文件夹时：**先改 JSON 别名，再跑绑皮**；能写成模式规则的优先进 `db.py`，避免别名表无限膨胀。
...
| 符号 | 文件 |
|------|------|
| `meta_alter_slug` / `is_meta_display_name` | `roster/db.py` |
| `yi_num_folder_slug` / `u_boat_folder_slug` | `roster/db.py` |
| `mu_idol_folder_slug` / `xiao_younv_folder_slug` | `roster/db.py` |
| `variant_character_name_for_suffix` | `roster/db.py` |
| `resolve_bind_skin_id`（doa/idol/younv/h→oath） | `roster/db.py` |
| `bind_oath_h_pets` | `roster/db.py` |
| `enrich_alias_map_from_roster` | `roster/db.py` |
