...
fn run_menu_switch_blocking(
    ...
    let engine = {
        let db = crate::db::lock_conn(&st.db)?;
        crate::pet::get_companion_engine(&db)
    };
    if engine == crate::pet::COMPANION_ENGINE_KANMUSU && result.kanmusu_ready {
    ...
    if result.spine_ready && !result.model_id.trim().is_empty() {
        return crate::pet::menu_switch_to_model_blocking(app, st, &result.model_id, timeout_ms);
    }
...
fn run_menu_switch_character_blocking(
    ...
    if engine == crate::pet::COMPANION_ENGINE_KANMUSU && kanmusu_ready {
        crate::kanmusu::desktop_open(app, &st, character_id, &skin_id)?;
        return Ok(kanmusu_dir.unwrap_or(skin_id));
    }
    if spine_ready && !model_id.trim().is_empty() {
        return crate::pet::menu_switch_to_model_blocking(app, st, &model_id, timeout_ms);
    }
    if kanmusu_ready {
        crate::kanmusu::desktop_open(app, &st, character_id, &skin_id)?;
        return Ok(kanmusu_dir.unwrap_or(skin_id));
...
fn pick_next_skin(st: &AppState) -> Result<(String, String), String> {
    ...
    let skins: Vec<_> = menu
        .skins
        .iter()
        .filter(|s| s.model_ready || s.kanmusu_ready)
        .collect();
    if skins.is_empty() {
