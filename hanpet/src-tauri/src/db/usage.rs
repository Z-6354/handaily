...
fn close_open_session(db: &Connection, table: &str) -> Result<(), rusqlite::Error> {
    ...
    let mut stmt = db.prepare(&format!(
        "SELECT id, started_at FROM {table} WHERE ended_at IS NULL"
    ))?;
    ...
    for (id, started_at) in rows {
        let duration_ms = crate::db::segment_duration_ms(&started_at, &now);
        db.execute(
            &format!("UPDATE {table} SET ended_at = ?1, duration_ms = ?2 WHERE id = ?3"),
            rusqlite::params![now, duration_ms as i64, id],
        )?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn recover_orphan_preserves_companion_duration() {
        let db = test_db();
        let started_dt = Local::now() - chrono::Duration::hours(2);
        let started = started_dt.to_rfc3339();
        db.execute(
        ...
        let ms = companion_ms_for_date(&db, started_dt.date_naive()).unwrap();
