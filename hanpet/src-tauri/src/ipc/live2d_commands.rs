...
pub async fn characters_set_active(
    ...
    let data_dir = st.data_dir();
    let (skin_id, model_id, spine_ready, kanmusu_ready, kanmusu_dir) = {
        let db = crate::db::lock_conn(&st.db)?;
        let manifest = crate::persona::load_manifest(data_dir);
    ...
    // Prefer Spine when ready; otherwise open Cubism companion.
    if spine_ready && !model_id.trim().is_empty() {
        crate::pet::set_active_model(&app, st.inner().clone(), &model_id)?;
        let db = crate::db::lock_conn(&st.db)?;
        return Ok(crate::pet::models::active_model_id(&db));
    }
    if kanmusu_ready {
        crate::kanmusu::desktop_open(&app, st.inner(), &character_id, &skin_id)?;
...
pub async fn characters_set_skin(
    ...
    // Validate explicit companion before writing prefs, so hard-fail leaves state unchanged.
    if mode == "spine" || mode == "kanmusu" {
        let preview = crate::character::get_skin_detail(data_dir, &character_id, &skin_id)?;
        if mode == "spine" && (!preview.model_ready || preview.model_id.trim().is_empty()) {
            return Err("该皮肤桌宠模型尚未完成，请先导入 Spine 模型".into());
        }
        if mode == "kanmusu" && !preview.kanmusu_ready {
            return Err("该皮肤舰娘模型尚未完成，请先导入 Cubism 模型".into());
        }
    }
    let result = {
        let db = crate::db::lock_conn(&st.db)?;
    ...
    if mode == "spine" {
        crate::pet::set_active_model(&app, st.inner().clone(), &result.model_id)?;
        let db = crate::db::lock_conn(&st.db)?;
        return Ok(crate::pet::models::active_model_id(&db));
    }
    if mode == "kanmusu" {
        crate::kanmusu::desktop_open(&app, st.inner(), &character_id, &skin_id)?;
        return Ok(result.kanmusu_dir.unwrap_or(skin_id));
    }

    // auto: respect current companion engine when kanmusu preferred & ready
    let prefer_kanmusu = {
        let db = crate::db::lock_conn(&st.db)?;
        crate::pet::get_companion_engine(&db) == crate::pet::COMPANION_ENGINE_KANMUSU
            && result.kanmusu_ready
    };
    if prefer_kanmusu {
        crate::kanmusu::desktop_open(&app, st.inner(), &character_id, &skin_id)?;
        return Ok(result.kanmusu_dir.unwrap_or(skin_id));
...
#[tauri::command]
pub async fn characters_get_skin(
    st: State<'_, Arc<AppState>>,
    character_id: String,
    skin_id: String,
) -> Result<crate::character::CharacterSkinDetail, String> {
    crate::character::get_skin_detail(st.data_dir(), &character_id, &skin_id)
}

#[tauri::command]
pub async fn characters_update_skin_lines(
    st: State<'_, Arc<AppState>>,
    character_id: String,
    skin_id: String,
    lines: Vec<crate::character::CharacterSkinLine>,
) -> Result<crate::character::CharacterSkinDetail, String> {
    crate::character::update_skin_lines(st.data_dir(), &character_id, &skin_id, lines)
}

#[tauri::command]
pub async fn characters_pet_menu_kanmusu(
    st: State<'_, Arc<AppState>>,
) -> Result<Vec<crate::character::CharacterBrief>, String> {
    let db = crate::db::lock_conn(&st.db)?;
    Ok(crate::character::list_characters_with_kanmusu(
        st.data_dir(),
        &db,
    ))
}
...
#[tauri::command]
pub fn pet_click_through_poll(
    app: tauri::AppHandle,
) -> Result<crate::pet::PetClickThroughPoll, String> {
    crate::pet::click_through_poll(&app)
}

#[tauri::command]
pub fn pet_move_noactivate(app: tauri::AppHandle, x: i32, y: i32) -> Result<(), String> {
    crate::pet::move_pet_position_noactivate(&app, x, y)
}
...
pub async fn pet_menu_switch_skin(
    ...
    crate::pet::menu_switch_skin(
        ...
        prefer_engine.as_deref(),
    )
    .await
}

#[tauri::command]
pub fn pet_set_hit_areas_visible(app: tauri::AppHandle, visible: bool) -> Result<(), String> {
    crate::pet::set_hit_areas_visible(&app, visible)
}
...
#[tauri::command]
pub async fn pet_delete_all_user_models(
    app: tauri::AppHandle,
    st: State<'_, Arc<AppState>>,
) -> Result<PetDeleteAllResult, String> {
    let data_dir = st.data_dir();
    let deleted = {
        let db = crate::db::lock_conn(&st.db)?;
        let ids = crate::pet::models::delete_all_user_models(data_dir, &db)?;
        ids
    };
    for id in &deleted {
        let _ = crate::character::purge_model_from_manifest(data_dir, id);
    }
    let active = {
        let db = crate::db::lock_conn(&st.db)?;
        crate::pet::models::active_model_id(&db)
    };
    if deleted.iter().any(|id| id == &active) || !crate::pet::models::is_builtin_model(&active) {
        // 若当前激活被删或不在内置列表，切回柴郡
        if deleted.iter().any(|id| id == &active) {
            crate::pet::set_active_model(
                &app,
                st.inner().clone(),
    ...
    Ok(PetDeleteAllResult {
        deleted,
        count,
        message: format!("已删除 {count} 个非内置桌宠模型"),
    })
}
