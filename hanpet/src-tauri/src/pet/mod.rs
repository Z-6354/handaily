...
pub struct PetRuntimeState {
    ...
    /// 当前 `pet` 窗已加载的 companion 页面（`spine` | `kanmusu`）
    pub loaded_companion_engine: Mutex<String>,
}
...
pub fn get_scale(db: &rusqlite::Connection) -> f64 {
    ensure_engine_layout_seeded(db);
    let key = if get_companion_engine(db) == COMPANION_ENGINE_KANMUSU {
        "kanmusu_scale"
    } else {
        "pet_scale"
    };
    let v: f64 = crate::db::get_setting(db, key)
...
pub const COMPANION_ENGINE_SPINE: &str = "spine";
pub const COMPANION_ENGINE_KANMUSU: &str = "kanmusu";

pub fn normalize_companion_engine(engine: &str) -> &'static str {
    if engine.eq_ignore_ascii_case(COMPANION_ENGINE_KANMUSU) {
        COMPANION_ENGINE_KANMUSU
    } else {
        COMPANION_ENGINE_SPINE
    }
}

pub fn get_companion_engine(db: &rusqlite::Connection) -> String {
    match crate::db::get_setting(db, "companion_engine").as_deref() {
        Some(v) if v.eq_ignore_ascii_case(COMPANION_ENGINE_KANMUSU) => {
            COMPANION_ENGINE_KANMUSU.to_string()
        }
        _ => COMPANION_ENGINE_SPINE.to_string(),
    }
}

pub fn set_companion_engine(db: &rusqlite::Connection, engine: &str) -> Result<(), String> {
    crate::db::set_setting(db, "companion_engine", normalize_companion_engine(engine))
        .map_err(|e| e.to_string())
}

pub fn get_kanmusu_active_character_id(db: &rusqlite::Connection) -> Option<String> {
    crate::db::get_setting(db, "kanmusu_active_character_id").filter(|s| !s.trim().is_empty())
}

pub fn get_kanmusu_active_skin_id(db: &rusqlite::Connection) -> Option<String> {
    crate::db::get_setting(db, "kanmusu_active_skin_id").filter(|s| !s.trim().is_empty())
}

pub fn set_kanmusu_active_ids(
    db: &rusqlite::Connection,
    character_id: &str,
    skin_id: &str,
) -> Result<(), String> {
    crate::db::set_setting(db, "kanmusu_active_character_id", character_id.trim())
        .map_err(|e| e.to_string())?;
    crate::db::set_setting(db, "kanmusu_active_skin_id", skin_id.trim())
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
/// 调整 Z 序。`force_show=true` 仅用于桌宠从全屏后强制露面；**绝不能**用于菜单窗，
/// 否则 SWP_SHOWWINDOW 会在「右键才该出现」的菜单上把窗口常驻显示进来。
fn apply_webview_topmost(win: &WebviewWindow, topmost: bool) {
    apply_webview_topmost_ex(win, topmost, false);
}

fn apply_webview_topmost_ex(win: &WebviewWindow, topmost: bool, force_show: bool) {
    #[cfg(windows)]
    {
        ...
        if let Ok(hwnd) = hwnd {
            unsafe {
                ...
                let _ = SetWindowPos(hwnd, Some(z), 0, 0, 0, 0, flags);
            }
        }
    }
...
fn clear_fullscreen_suppression_and_restore(app: &AppHandle, st: &Arc<AppState>) {
    ...
    // 抑制时发过 pet-hidden，退出全屏必须 resume，否则画面停在 paused
...
pub fn set_scale(db: &rusqlite::Connection, scale: f64) -> Result<(), String> {
    ...
    crate::db::set_setting(db, key, &s.to_string()).map_err(|e| e.to_string())
}
...
pub fn get_window_size(db: &rusqlite::Connection) -> (f64, f64) {
    ensure_engine_layout_seeded(db);
    let (wk, hk) = if get_companion_engine(db) == COMPANION_ENGINE_KANMUSU {
        ("kanmusu_width", "kanmusu_height")
    } else {
        ("pet_width", "pet_height")
    };
    let w: f64 = crate::db::get_setting(db, wk)
        .and_then(|s| s.parse().ok())
        .unwrap_or(DEFAULT_WIDTH);
    let h: f64 = crate::db::get_setting(db, hk)
        .and_then(|s| s.parse().ok())
...
/// Cubism 模型平移（Pixi 坐标）；与 Spine CSS `pet_offset_*` 隔离，避免引擎互串
pub fn get_kanmusu_model_offset(db: &rusqlite::Connection) -> (f64, f64) {
    let x: f64 = crate::db::get_setting(db, "kanmusu_offset_x")
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.0);
    let y: f64 = crate::db::get_setting(db, "kanmusu_offset_y")
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.0);
    (x, y)
}

fn save_engine_model_offset(
    db: &rusqlite::Connection,
    offset_x: f64,
    offset_y: f64,
) -> Result<(), String> {
    let x = offset_x.round().to_string();
    let y = offset_y.round().to_string();
    if get_companion_engine(db) == COMPANION_ENGINE_KANMUSU {
        crate::db::set_setting(db, "kanmusu_offset_x", &x).map_err(|e| e.to_string())?;
        crate::db::set_setting(db, "kanmusu_offset_y", &y).map_err(|e| e.to_string())?;
    } else {
        crate::db::set_setting(db, "pet_offset_x", &x).map_err(|e| e.to_string())?;
        crate::db::set_setting(db, "pet_offset_y", &y).map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
pub fn save_window_size(db: &rusqlite::Connection, width: f64, height: f64) -> Result<(), String> {
    ...
    crate::db::set_setting(db, wk, &w.to_string()).map_err(|e| e.to_string())?;
    crate::db::set_setting(db, hk, &h.to_string()).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn save_layout(
    ...
    set_scale(db, scale)?;
    save_engine_model_offset(db, offset_x, offset_y)?;
    let pos = load_position(db);
    save_position(db, pos.0, pos.1, position_win_w, position_win_h)?;
    Ok(())
}

const COMPANION_LAYOUT_SEED_V: &str = "companion_layout_seed_v";
const COMPANION_LAYOUT_SEED_CURRENT: &str = "2";

/// 舰娘布局键若尚无值：窗位/尺寸可从桌宠拷贝；缩放独立默认 0.8（勿拷贝 pet_scale）。
/// v1 曾把 pet_scale 拷到 kanmusu_scale：一次性纠正「二者数值相同」的错误种子。
fn ensure_engine_layout_seeded(db: &rusqlite::Connection) {
    let seed_ver = crate::db::get_setting(db, COMPANION_LAYOUT_SEED_V);
    if seed_ver.as_deref() != Some(COMPANION_LAYOUT_SEED_CURRENT) {
        let pet_s = crate::db::get_setting(db, "pet_scale");
        let kan_s = crate::db::get_setting(db, "kanmusu_scale");
        match (&kan_s, &pet_s) {
            (None, _) => {
                let _ = crate::db::set_setting(db, "kanmusu_scale", "0.8");
            }
            (Some(k), Some(p)) if k == p => {
                // v1 拷贝特征：与桌宠完全相同 → 恢复舰娘独立默认
                let _ = crate::db::set_setting(db, "kanmusu_scale", "0.8");
            }
            _ => {}
        }
        let _ = crate::db::set_setting(db, COMPANION_LAYOUT_SEED_V, COMPANION_LAYOUT_SEED_CURRENT);
    } else if crate::db::get_setting(db, "kanmusu_scale").is_none() {
        let _ = crate::db::set_setting(db, "kanmusu_scale", "0.8");
    }

    let needs_geom = crate::db::get_setting(db, "kanmusu_width").is_none()
        || crate::db::get_setting(db, "kanmusu_x").is_none();
    if !needs_geom {
        return;
    }
    let w = crate::db::get_setting(db, "pet_width").unwrap_or_else(|| DEFAULT_WIDTH.to_string());
    let h = crate::db::get_setting(db, "pet_height").unwrap_or_else(|| DEFAULT_HEIGHT.to_string());
    let x = crate::db::get_setting(db, "pet_x").unwrap_or_else(|| default_position().0.to_string());
    let y = crate::db::get_setting(db, "pet_y").unwrap_or_else(|| default_position().1.to_string());
    let _ = crate::db::set_setting(db, "kanmusu_width", &w);
    let _ = crate::db::set_setting(db, "kanmusu_height", &h);
    let _ = crate::db::set_setting(db, "kanmusu_x", &x);
    let _ = crate::db::set_setting(db, "kanmusu_y", &y);
}

fn pet_webview_url(engine: &str) -> WebviewUrl {
    // 必须 App 协议：External(localhost) 为 remote origin，pet-capability 仅授权 local，
    // invoke 被拒 → 透明空窗（见 docs/questions/62、67）
    if normalize_companion_engine(engine) == COMPANION_ENGINE_KANMUSU {
...
/// Ensure pet window HTML matches `companion_engine` (recreate if needed).
pub fn sync_pet_companion_page(app: &AppHandle, st: &AppState) -> Result<(), String> {
    let engine = {
        let db = crate::db::lock_conn(&st.db)?;
        get_companion_engine(&db)
    };
    let loaded = app
        .try_state::<PetRuntimeState>()
        .and_then(|rt| rt.loaded_companion_engine.lock().ok().map(|g| g.clone()))
        .unwrap_or_default();
    let win_exists = app.get_webview_window(PET_LABEL).is_some();
    if win_exists && loaded == engine {
        return Ok(());
    }
    if win_exists {
...
/// Persist engine and rebuild pet page when switching Spine ↔ Cubism.
pub fn apply_companion_engine(
    app: &AppHandle,
    st: &AppState,
    engine: &str,
) -> Result<(), String> {
    let engine = normalize_companion_engine(engine).to_string();
    {
        let db = crate::db::lock_conn(&st.db)?;
        ensure_engine_layout_seeded(&db);
        set_companion_engine(&db, &engine)?;
    }
    sync_pet_companion_page(app, st)?;
    // 切引擎：立刻套用该引擎上次窗位 / 尺寸 / 缩放
    apply_current_engine_layout(app, st)?;
    Ok(())
}

/// 按当前 companion_engine 恢复窗布局，并通知前端刷新 scale。
pub fn apply_current_engine_layout(app: &AppHandle, st: &AppState) -> Result<(), String> {
    let (w, h, x, y, scale) = {
        let db = crate::db::lock_conn(&st.db)?;
        ensure_engine_layout_seeded(&db);
        let (w, h) = get_window_size(&db);
        let (x, y) = load_position(&db);
        let scale = get_scale(&db);
        (w, h, x, y, scale)
    };
    if let Some(win) = app.get_webview_window(PET_LABEL) {
        ...
        let (cx, cy) = resolve_pet_position(x, y, resolve_w, resolve_h);
        {
            let db = crate::db::lock_conn(&st.db)?;
            let _ = save_position(&db, cx, cy, Some(resolve_w), Some(resolve_h));
        }
        if let Some(rt) = app.try_state::<PetRuntimeState>() {
            let _ = set_pet_position(&win, &rt, cx, cy);
        }
    }
    nudge_pet_scale(app, scale);
    Ok(())
}
...
pub fn ensure_pet_menu_window(app: &AppHandle) -> Result<(), String> {
    ...
    if let Some(win) = app.get_webview_window(PET_MENU_LABEL) {
        ...
        // 预建菜单窗保持隐藏；仅右键 show_pet_menu 才露面
        let _ = win.hide();
    }
    Ok(())
}
...
fn emit_pet_menu_state(app: &AppHandle, open: bool) {
    ...
    // 独立预览窗右键菜单时也要同步状态
    let _ = app.emit_to(
        crate::kanmusu::KANMUSU_PLAYER_LABEL,
        "pet-menu-state",
        open,
    );
}
...
pub fn hide_pet_menu(app: &AppHandle) -> Result<(), String> {
    ...
    if let Some(win) = app.get_webview_window(PET_MENU_LABEL) {
        // 先 hide 再动 Z 序，避免 topmost 路径误 SHOWWINDOW
        let _ = win.hide();
...
pub fn enter_pet_edit_bounds(app: &AppHandle) -> Result<(), String> {
    ...
    let engine = app
        ...
        .and_then(|st| {
            crate::db::lock_conn(&st.db)
                .ok()
                .map(|db| get_companion_engine(&db))
        })
        .unwrap_or_else(|| COMPANION_ENGINE_SPINE.to_string());

    // 独立「舰娘预览」窗在前时：事件必须发到 kanmusu-player，且不能 player_close 把它干掉
    let preview_visible = app
        .get_webview_window(crate::kanmusu::KANMUSU_PLAYER_LABEL)
        .and_then(|w| w.is_visible().ok())
        .unwrap_or(false);
    if preview_visible {
        let _ = app.emit_to(
            crate::kanmusu::KANMUSU_PLAYER_LABEL,
            "pet-enter-edit-bounds",
            (),
        );
        hide_pet_menu(app)?;
        if let Some(win) = app.get_webview_window(crate::kanmusu::KANMUSU_PLAYER_LABEL) {
    ...
    // 先发事件给前端，再关菜单，避免 pet-menu-state 抢先 startClickThrough
    let _ = app.emit_to(PET_LABEL, "pet-enter-edit-bounds", ());
    hide_pet_menu(app)?;
    if let Some(win) = app.get_webview_window(PET_LABEL) {
        // Spine：编辑框需整窗接事件。舰娘为「点击模式」toggle，穿透由前端决定，
        // 不可在此强制 false，否则第二次点菜单「编辑范围」无法恢复穿透 → 桌面挡层。
        if engine == COMPANION_ENGINE_SPINE {
            let _ = win.set_ignore_cursor_events(false);
        }
        let _ = win.show();
        let _ = win.set_focus();
    }
    // 桌宠编辑路径：清掉残留独立预览层，防挡点击
    let _ = crate::kanmusu::player_close(app);
    sync_pet_topmost(app);
    Ok(())
}
...
/// 舰娘/桌宠穿透轮询：cursor + 窗体 + 键态一次返回
#[derive(Clone, Serialize)]
pub struct PetClickThroughPoll {
    pub cursor_x: f64,
    pub cursor_y: f64,
    pub win_x: i32,
    pub win_y: i32,
...
pub fn click_through_poll(app: &AppHandle) -> Result<PetClickThroughPoll, String> {
    let cursor = app.cursor_position().map_err(|e| e.to_string())?;
    let bounds = get_pet_window_bounds(app)?;
    let scale_factor = app
        .get_webview_window(PET_LABEL)
        .and_then(|w| w.scale_factor().ok())
        .unwrap_or(1.0);
    Ok(PetClickThroughPoll {
        cursor_x: cursor.x,
        cursor_y: cursor.y,
        win_x: bounds.x,
        win_y: bounds.y,
        ...
        left_down: is_left_mouse_down(),
        right_down: is_right_mouse_down(),
        scale_factor,
    })
}
...
/// 拖动挪窗：仅改位置、不激活、不动 Z 序（避免全屏游戏下抢焦把任务栏顶出）
pub fn move_pet_position_noactivate(app: &AppHandle, x: i32, y: i32) -> Result<(), String> {
    let win = app
        .get_webview_window(PET_LABEL)
        .ok_or_else(|| "桌宠窗口不存在".to_string())?;
    let bounds = read_pet_window_bounds_from_win(&win)?;
    set_pet_window_bounds_on_win(
        &win,
        x,
        y,
        bounds.width,
...
pub fn get_config(st: &AppState) -> Result<PetConfigPayload, String> {
    let db = crate::db::lock_conn(&st.db)?;
    if get_companion_engine(&db) == COMPANION_ENGINE_KANMUSU {
        // 舰娘模式不依赖 Spine resolve_assets；只回布局/缩放/偏移
        let (window_width, window_height) = get_window_size(&db);
        let (offset_x, offset_y) = get_kanmusu_model_offset(&db);
        let skin = get_kanmusu_active_skin_id(&db).unwrap_or_default();
        return Ok(PetConfigPayload {
            model_id: skin,
            model_name: String::new(),
            asset_base: String::new(),
            config_file: None,
            skel_file: String::new(),
...
pub fn get_config_for_model(
    ...
    let (window_width, window_height) = get_window_size(db);
    let (offset_x, offset_y) = if get_companion_engine(db) == COMPANION_ENGINE_KANMUSU {
        get_kanmusu_model_offset(db)
    } else {
        get_model_offset(db)
    };
    Ok(PetConfigPayload {
...
fn load_position(db: &rusqlite::Connection) -> (i32, i32) {
    ensure_engine_layout_seeded(db);
    let (xk, yk) = if get_companion_engine(db) == COMPANION_ENGINE_KANMUSU {
        ("kanmusu_x", "kanmusu_y")
    } else {
        ("pet_x", "pet_y")
    };
    let x = crate::db::get_setting(db, xk)
        .and_then(|s| s.parse().ok())
        .unwrap_or_else(|| default_position().0);
    let y = crate::db::get_setting(db, yk)
        .and_then(|s| s.parse().ok())
...
pub fn save_position(
    ...
    crate::db::set_setting(db, xk, &x.to_string()).map_err(|e| e.to_string())?;
    crate::db::set_setting(db, yk, &y.to_string()).map_err(|e| e.to_string())?;
    Ok(PetPoint { x, y })
}
...
pub fn ensure_pet_window(app: &AppHandle, st: &AppState) -> Result<(), String> {
    ...
    fn create(app: &AppHandle, st: &AppState) -> Result<(), String> {
        ...
        if let Some(rt) = app.try_state::<PetRuntimeState>() {
            if let Ok(mut g) = rt.loaded_companion_engine.lock() {
                *g = engine.clone();
            }
        }
        ...
        WebviewWindowBuilder::new(app, PET_LABEL, pet_webview_url(&engine))
            .title("小寒桌宠")
            .inner_size(w, h)
            .transparent(true)
            .decorations(false)
            ...
            .on_page_load(move |_window, payload| {
                ...
                if let Some(rt) = app_for_load.try_state::<PetRuntimeState>() {
                    ...
                    if engine_for_load == COMPANION_ENGINE_KANMUSU {
                        if let Some(krt) = app_for_load.try_state::<crate::kanmusu::KanmusuRuntimeState>()
                        {
                            krt.page_load_finished.store(true, Ordering::Release);
                            // 只 emit、不 take：bootstrap 的 consume_pending 仍能取到，
                            // 避免 listen 未就绪时丢事件 → 透明空窗。
                            if let Ok(pending) = krt.pending_load.lock() {
                                if let Some(load) = pending.as_ref() {
...
fn reset_pet_runtime_state(app: &AppHandle) {
    if let Some(rt) = app.try_state::<PetRuntimeState>() {
        ...
        if let Ok(mut guard) = rt.loaded_companion_engine.lock() {
            guard.clear();
        }
        rt.fullscreen_suppressed.store(false, Ordering::Relaxed);
        rt.user_hidden_pet.store(false, Ordering::Release);
    }
    if let Some(krt) = app.try_state::<crate::kanmusu::KanmusuRuntimeState>() {
        krt.page_load_finished.store(false, Ordering::Release);
    }
}
...
pub fn show_pet(app: &AppHandle, st: &Arc<AppState>) -> Result<(), String> {
    ...
    if engine == COMPANION_ENGINE_SPINE {
        ...
        // Startup / show: restore last kanmusu skin if nothing queued yet.
        let has_pending = app
            .try_state::<crate::kanmusu::KanmusuRuntimeState>()
            .and_then(|k| k.pending_load.lock().ok().map(|g| g.is_some()))
            .unwrap_or(false);
        if !has_pending {
            let (cid, sid) = {
                let db = crate::db::lock_conn(&st.db)?;
                (
                    get_kanmusu_active_character_id(&db),
                    get_kanmusu_active_skin_id(&db),
                )
            };
            if let (Some(cid), Some(sid)) = (cid, sid) {
                if let Ok(payload) =
                    crate::kanmusu::build_player_load_payload(st.data_dir(), &cid, &sid)
                {
                    if let Some(krt) = app.try_state::<crate::kanmusu::KanmusuRuntimeState>() {
                        if let Ok(mut g) = krt.pending_load.lock() {
                            *g = Some(payload.clone());
                        }
                        if krt.page_load_finished.load(Ordering::Acquire)
                            || rt.page_load_finished.load(Ordering::Acquire)
                        {
    ...
    // 预建菜单后强制关：避免历史 SHOWWINDOW 残留导致常驻；仅右键再开
    let _ = hide_pet_menu(app);
    if let Some(rt) = app.try_state::<PetRuntimeState>() {
        rt.user_hidden_pet.store(false, Ordering::Release);
    }
...
pub fn menu_switch_to_model_blocking(
    ...
    // 舰娘页无法响应 Spine 的 pet-switch；或窗口未建时，走 set_active_model 重建 pet.html。
    if engine == COMPANION_ENGINE_KANMUSU || app.get_webview_window(PET_LABEL).is_none() {
...
pub async fn menu_switch_skin(
    ...
    let prefer = prefer_engine
        .map(normalize_companion_engine)
        .unwrap_or_else(|| normalize_companion_engine(&current));

    // 双栏 · 舰娘：跳过 select_character_skin（normalize + manifest 写仓），直接上桌
    if prefer == COMPANION_ENGINE_KANMUSU {
        let skin = crate::kanmusu::lookup_skin_detail_public(st.data_dir(), character_id, skin_id)?;
        if skin.model_ready {
            crate::kanmusu::desktop_open(app, &st, character_id, skin_id)?;
            return Ok(skin.model_dir);
        }
    ...
    if prefer == COMPANION_ENGINE_SPINE && result.spine_ready && !result.model_id.trim().is_empty() {
        return menu_switch_to_model(app, st, &result.model_id, timeout_ms).await;
    }
    // 回退：另一栏可用则切过去
    if result.kanmusu_ready {
        crate::kanmusu::desktop_open(app, &st, character_id, skin_id)?;
        return Ok(result.kanmusu_dir.unwrap_or_else(|| skin_id.to_string()));
    }
    if result.spine_ready && !result.model_id.trim().is_empty() {
        return menu_switch_to_model(app, st, &result.model_id, timeout_ms).await;
    }
    Err("该皮肤模型尚未完成，请先导入桌宠或舰娘模型".into())
}

/// 舰娘桌宠会话：切换点击区域 overlay（不落库，默认关）
pub fn set_hit_areas_visible(app: &AppHandle, visible: bool) -> Result<(), String> {
    let _ = app.emit_to(PET_LABEL, "pet-hit-areas-visible", visible);
    let _ = app.emit_to(
        crate::kanmusu::KANMUSU_PLAYER_LABEL,
        "pet-hit-areas-visible",
        visible,
    );
    Ok(())
}
...
pub async fn menu_switch_character(
    ...
    let (skin_id, model_id, spine_ready, kanmusu_ready, kanmusu_dir, engine) = {
        let db = crate::db::lock_conn(&st.db)?;
        ...
        let skin = crate::character::get_skin_detail(data_dir, character_id, &detail.active_skin_id)?;
        (
            detail.active_skin_id,
            skin.model_id,
            skin.model_ready,
            skin.kanmusu_ready,
            skin.kanmusu_dir,
    ...
    // 与 menu_switch_skin 一致：优先当前 companion_engine
    if engine == COMPANION_ENGINE_KANMUSU && kanmusu_ready {
        crate::kanmusu::desktop_open(app, &st, character_id, &skin_id)?;
        return Ok(kanmusu_dir.unwrap_or(skin_id));
    }
    if spine_ready && !model_id.trim().is_empty() {
        return menu_switch_to_model(app, st, &model_id, timeout_ms).await;
    }
    if kanmusu_ready {
        crate::kanmusu::desktop_open(app, &st, character_id, &skin_id)?;
        return Ok(kanmusu_dir.unwrap_or(skin_id));
    ...
    let _ = timeout_ms;
    Err("该人物皮肤模型尚未完成，请先导入桌宠或舰娘模型".into())
}
...
pub fn set_active_model(app: &AppHandle, st: Arc<AppState>, model_id: &str) -> Result<(), String> {
    let data_dir = st.data_dir();
    let assets = models::resolve_assets(data_dir, model_id)?;
    let (enabled, was_kanmusu) = {
        let db = crate::db::lock_conn(&st.db)?;
        models::set_active_model_id(&db, &assets.model_id)?;
        crate::character::sync_from_model(data_dir, &db, &assets.model_id);
        let was_kanmusu = get_companion_engine(&db) == COMPANION_ENGINE_KANMUSU;
        if was_kanmusu {
            set_companion_engine(&db, COMPANION_ENGINE_SPINE)?;
        }
        (is_enabled(&db), was_kanmusu)
    };
    if was_kanmusu {
        // Spine 顶替舰娘：重建 pet.html 后显示，并套用桌宠布局
        let _ = crate::kanmusu::player_close(app);
        sync_pet_companion_page(app, &st)?;
        let _ = apply_current_engine_layout(app, &st);
    }
    if enabled {
        if was_kanmusu || app.get_webview_window(PET_LABEL).is_none() {
            show_pet(app, &st)?;
        } else {
            nudge_pet(app, &st);
        }
...
fn ensure_topmost_heartbeat(app: &AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            // 心跳顺带检测沉浸式全屏（不必等 remark 调度）
            if let Some(st) = app.try_state::<Arc<AppState>>() {
                sync_fullscreen_visibility(&app, &st);
            }
            let (user_hidden, fs_suppressed) = app
                .try_state::<PetRuntimeState>()
                .map(|rt| {
                    (
                        rt.user_hidden_pet.load(Ordering::Acquire),
                        rt.fullscreen_suppressed.load(Ordering::Relaxed),
                    )
                })
                .unwrap_or((false, false));
            if user_hidden || fs_suppressed {
                // 全屏抑制中：绝不 force show / TOPMOST / pet-resume（否则任务栏被顶出）
                continue;
            }
            // 非全屏时窗体可能被系统藏起：置顶开启且桌宠启用时强制 show + TOPMOST
            let should_show = app
                .try_state::<Arc<AppState>>()
                .and_then(|st| {
                    st.db.lock().ok().map(|db| is_enabled(&db) && is_always_on_top_enabled(&db))
                })
                .unwrap_or(false);
            if should_show && !pet_visible(&app) {
                ...
                // 仅桌宠强制露面；菜单仍靠右键 show，绝不 SHOWWINDOW
                if let Some(pet) = app.get_webview_window(PET_LABEL) {
                    if pet.is_visible().unwrap_or(false) {
                        apply_webview_topmost_ex(&pet, pet_should_be_topmost(&app), true);
                    }
                }
            }
            if !pet_visible(&app) {
            ...
            sync_pet_topmost(&app);
            // 菜单已显示时才抬层；隐藏菜单绝不再 SetWindowPos(SHOWWINDOW)
            sync_menu_z_order_if_visible(&app);
            // 全屏应用盖住 WebView 后，页面可能卡在 renderPaused；周期 nudge 恢复
            let _ = app.emit_to(PET_LABEL, "pet-resume", ());
        }
    });
}
...
fn enter_fullscreen_suppression(app: &AppHandle) {
    let Some(rt) = app.try_state::<PetRuntimeState>() else {
        return;
    };
    let was = rt.fullscreen_suppressed.swap(true, Ordering::Relaxed);
    let _ = hide_pet_menu(app);
    if let Some(pet) = app.get_webview_window(PET_LABEL) {
        // 先撤 TOPMOST 再 hide，避免置顶抢前台把任务栏「顶」出来
        apply_webview_topmost(&pet, false);
        let _ = pet.hide();
    }
    if !was {
...
/// 其它应用沉浸式全屏时隐藏桌宠（含「始终置顶」）；退出后恢复。
/// 此前 AOT=true 时完全跳过抑制 → 心跳 SHOWWINDOW/TOPMOST + 拖动抢焦导致任务栏闪现。
fn sync_fullscreen_visibility(app: &AppHandle, st: &Arc<AppState>) {
    #[cfg(windows)]
    {
        let immersive = crate::tracker::win32::is_foreground_fullscreen()
            && !crate::tracker::win32::is_foreground_own_process();
        if immersive {
            enter_fullscreen_suppression(app);
            return;
        }
    }
    clear_fullscreen_suppression_and_restore(app, st);
...
pub fn build_remark_from_lines(st: &AppState) -> Option<PetRemarkPayload> {
    ...
    // 舰娘上桌：台词来自 active kanmusu 皮肤，避免仍读 Spine 模型 lines
    if get_companion_engine(&db) == COMPANION_ENGINE_KANMUSU {
        return build_kanmusu_remark_from_lines(st, &db);
    }
    let model_id = models::active_model_id(&db);
    let anim_meta = models::read_animation_meta(st.data_dir(), &db, &model_id);
...
fn build_kanmusu_remark_from_lines(
    st: &AppState,
    db: &rusqlite::Connection,
