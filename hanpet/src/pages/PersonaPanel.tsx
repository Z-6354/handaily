...
export function PersonaPanel({ active = true }: { active?: boolean }) {
  ...
  // keep-alive：离开人物页时清空搜索，返回时回到完整列表
  useEffect(() => {
    if (active) return;
    setSearchInput("");
    setActiveQuery("");
  ...
  const switchSkin = async (
    ...
    try {
      await xiaohan.charactersSetSkin(characterId, skinId, companion);
      await refresh();
      if (detailId === characterId) {
        await refreshDetail(characterId);
        setSkinRefreshKey((k) => k + 1);
    ...
    return (
      <div className="persona-page persona-page--detail">
        ...
        <PersonaDetailPanel
          ...
          englishName={characterDetail?.english_name}
          cv={characterDetail?.cv}
