...
export function SettingsPanel() {
  ...
  const handleDeleteAllUserModels = async () => {
    setDeleteAllBusy(true);
    setSaveError(null);
    try {
      const result = await xiaohan.petDeleteAllUserModels();
      setDeleteAllOpen(false);
      setSaveError(successFeedback(result.message));
    } catch (e) {
      setSaveError(parseApiError(e, "删除模型"));
    } finally {
  ...
  return (
    <div className="pref-shell">
      <div className="pref-shell__inner">
        ...
        {isTauriRuntime() && (
          <SettingsSection title="桌宠数据" accent>
            <p className="hint-block">
              删除所有非内置 Spine 桌宠模型（用户导入的 .skel 包）。内置柴郡等不受影响；舰娘 Cubism 模型独立存储，不会被删除。
            </p>
            <button
              type="button"
              className="btn-danger btn-sm"
              disabled={saving || deleteAllOpen}
      ...
      {deleteAllOpen && (
        <div
          ...
          role="presentation"
          onClick={deleteAllBusy ? undefined : () => setDeleteAllOpen(false)}
        >
          <div
          ...
          >
            <div className="persona-delete-modal-body">
              <h3 className="persona-delete-modal-title">删除全部非内置模型</h3>
              <p className="persona-delete-modal-lead">
                确定删除所有用户导入的 Spine 桌宠模型吗？内置模型将保留。
              </p>
              <ul className="persona-delete-modal-list">
                <li>pet-models 目录中的用户模型将被永久删除</li>
                <li>人物 manifest 中对应皮肤引用将被清理</li>
                <li>舰娘 Cubism 模型（kanmusu-models）不受影响</li>
                <li>此操作不可恢复</li>
              </ul>
            </div>
            <div className="persona-delete-modal-foot modal-actions">
              ...
              <button
                ...
                className="btn-danger btn-sm"
                disabled={deleteAllBusy}
                onClick={() => void handleDeleteAllUserModels()}
              >
                {deleteAllBusy ? "删除中…" : "确认删除"}
              </button>
            </div>
          </div>
        </div>
