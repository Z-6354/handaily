...
async function ensureClickThroughDisabled() {
  // 菜单打开时仍保持轮询：Windows 透明像素常穿透，只能靠全局左键轮询关菜单
  clickThroughApplySerial += 1;
  ignoreCursorActive = false;
  await applyClickThrough(false, true);
  if (!clickThroughInterval && !document.hidden && !mainWindowVisible) {
    clickThroughInterval = window.setInterval(() => {
      if (document.hidden) return;
      void runClickThroughPollTick();
    }, CLICK_THROUGH_POLL_MS);
...
async function openPetMenu() {
  ...
  try {
    ...
    // 不等 pet-menu-state：事件丢失时 petMenuOpen 仍假 → 左键关菜单/捕获层全废
    if (open) {
      petMenuOpen = true;
      menuCloseSuppressUntil = Date.now() + 400;
      root.classList.add("menu-open-capture");
      void ensureClickThroughDisabled();
    } else {
      petMenuOpen = false;
      root.classList.remove("menu-open-capture");
    }
  } catch (err) {
...
async function setupPetEvents() {
  ...
  const unlistenMenuState = await listen<boolean>("pet-menu-state", (ev) => {
    ...
    if (petMenuOpen) {
      // 打开菜单后短暂抑制「左键关菜单」，避免右键抬起误关；之后靠轮询关
      menuCloseSuppressUntil = Date.now() + 400;
      root.classList.add("menu-open-capture");
      void ensureClickThroughDisabled();
      return;
