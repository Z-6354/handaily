...
const _SkeletonBinary36 = class {
  ...
  readSkeletonData(binary) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      data.transformMode = _SkeletonBinary36.TransformModeValues[input.readInt(true)];
    ...
    for (let i = 0; i < n; i++) {
      ...
      data.blendMode = _SkeletonBinary36.BlendModeValues[input.readInt(true)];
    ...
    for (let i = 0, nn; i < n; i++) {
      ...
      data.positionMode = _SkeletonBinary36.PositionModeValues[input.readInt(true)];
      data.spacingMode = _SkeletonBinary36.SpacingModeValues[input.readInt(true)];
      data.rotateMode = _SkeletonBinary36.RotateModeValues[input.readInt(true)];
      data.offsetRotation = input.readFloat();
    ...
    const defaultSkin = this.readSkin36(input, skeletonData, "default", nonessential);
    if (defaultSkin != null) {
    ...
    n = input.readInt(true);
    for (let i = 0; i < n; i++) {
      const skin = this.readSkin36(
        input,
        skeletonData,
        input.readString(),
        nonessential,
      );
      if (skin != null) skeletonData.skins.push(skin);
    }
    ...
    n = input.readInt(true);
    for (let i = 0; i < n; i++) {
      const data = new EventData(input.readString());
  ...
  readSkin36(input, skeletonData, skinName, nonessential) {
  ...
  readAttachment(input, skeletonData, skin, slotIndex, attachmentName, nonessential) {
    ...
    const type = _SkeletonBinary36.AttachmentTypeValues[typeIndex];
    switch (type) {
  ...
  readAnimation(input, name, skeletonData) {
    ...
    for (let i = 0, n = input.readInt(true); i < n; i++) {
      ...
      for (let ii = 0, nn = input.readInt(true); ii < nn; ii++) {
        ...
        switch (timelineType) {
          case _SkeletonBinary36.SLOT_ATTACHMENT: {
          ...
          case _SkeletonBinary36.SLOT_COLOR: {
          ...
          case _SkeletonBinary36.SLOT_TWO_COLOR: {
    ...
    for (let i = 0, n = input.readInt(true); i < n; i++) {
      ...
      for (let ii = 0, nn = input.readInt(true); ii < nn; ii++) {
        ...
        const frameCount = input.readInt(true);
        switch (timelineType) {
          case _SkeletonBinary36.BONE_ROTATE: {
          ...
          case _SkeletonBinary36.BONE_TRANSLATE:
          case _SkeletonBinary36.BONE_SCALE:
          case _SkeletonBinary36.BONE_SHEAR: {
            let timeline;
            let timelineScale = 1;
            if (timelineType == _SkeletonBinary36.BONE_SCALE)
              timeline = new ScaleTimeline(frameCount);
            else if (timelineType == _SkeletonBinary36.BONE_SHEAR)
    ...
    for (let i = 0, n = input.readInt(true); i < n; i++) {
      ...
      for (let frameIndex = 0; frameIndex < frameCount; frameIndex++) {
        ...
        timeline.setFrame(frameIndex, time, mix, 0, bendDirection, false, false);
        if (frameIndex < frameCount - 1)
          this.readCurve(input, frameIndex, timeline);
      }
      timelines.push(timeline);
    ...
    for (let i = 0, n = input.readInt(true); i < n; i++) {
      ...
      for (let ii = 0, nn = input.readInt(true); ii < nn; ii++) {
        ...
        const frameCount = input.readInt(true);
        switch (timelineType) {
          case _SkeletonBinary36.PATH_POSITION:
          case _SkeletonBinary36.PATH_SPACING: {
            let timeline;
            let timelineScale = 1;
            if (timelineType == _SkeletonBinary36.PATH_SPACING) {
          ...
          case _SkeletonBinary36.PATH_MIX: {
  ...
  readCurve(input, frameIndex, timeline) {
    switch (input.readByte()) {
      case _SkeletonBinary36.CURVE_STEPPED:
        timeline.setStepped(frameIndex);
        break;
      case _SkeletonBinary36.CURVE_BEZIER:
...
let SkeletonBinary36 = _SkeletonBinary36;
SkeletonBinary36.AttachmentTypeValues = [
  0,
  1,
  2,
  3,
...
SkeletonBinary36.TransformModeValues = [
...
SkeletonBinary36.PositionModeValues = [PositionMode.Fixed, PositionMode.Percent];
SkeletonBinary36.SpacingModeValues = [SpacingMode.Length, SpacingMode.Fixed, SpacingMode.Percent];
SkeletonBinary36.RotateModeValues = [RotateMode.Tangent, RotateMode.Chain, RotateMode.ChainScale];
SkeletonBinary36.BlendModeValues = [BLEND_MODES.NORMAL, BLEND_MODES.ADD, BLEND_MODES.MULTIPLY, BLEND_MODES.SCREEN];
SkeletonBinary36.BONE_ROTATE = 0;
SkeletonBinary36.BONE_TRANSLATE = 1;
SkeletonBinary36.BONE_SCALE = 2;
SkeletonBinary36.BONE_SHEAR = 3;
SkeletonBinary36.SLOT_ATTACHMENT = 0;
SkeletonBinary36.SLOT_COLOR = 1;
SkeletonBinary36.SLOT_TWO_COLOR = 2;
SkeletonBinary36.PATH_POSITION = 0;
SkeletonBinary36.PATH_SPACING = 1;
SkeletonBinary36.PATH_MIX = 2;
SkeletonBinary36.CURVE_LINEAR = 0;
SkeletonBinary36.CURVE_STEPPED = 1;
SkeletonBinary36.CURVE_BEZIER = 2;
class LinkedMesh {
