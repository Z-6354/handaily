...
async function fetchRosterPage(
  ...
  try {
    return await xiaohan.charactersListPage({
      ...
      // 始终传 favoriteIds，供后端把收藏角色置顶（不仅限于「收藏」筛选）
      favoriteIds: favoriteIds.length > 0 ? favoriteIds : undefined,
    });
  } catch (e) {
    if (attempt < 1) {
...
export function useCharacterRoster(options: {
  ...
  favoritesOnly: boolean;
  favoriteIds: string[];
  /** 收藏尚未从设置载入时保持 false，避免首屏未置顶 */
  ready?: boolean;
  onAvatarsCached?: (paths: Record<string, string>) => void;
}) {
  const { query, page, pageSize, favoritesOnly, favoriteIds, ready = true, onAvatarsCached } = options;
  const favoriteKey = favoriteIds.join("\0");
