...
function AppShell() {
  ...
  useEffect(() => {
    ...
    void waitForTauriInternals()
      .then(() => listen<string>("main-navigate", (ev) => {
        ...
        const id = raw === "pet" || raw === "kanmusu" ? "persona" : raw;
  ...
  return (
    <div className="app">
      ...
      <main
        className={`content${
          page === "persona" ? " content--persona" : " content--scroll content--secondary"
        }`}
      >
        ...
        {mounted.persona && (
          <PageSlot active={page === "persona"}>
            <LazyPage>
              <PersonaPanel active={page === "persona"} />
            </LazyPage>
          </PageSlot>
        )}
        {mounted.settings && (
